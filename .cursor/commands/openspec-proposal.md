...
1. Extract the user's request:
   - If this prompt includes user input inside the `<UserRequest>` block above (populated by slash-command arguments), use that as the primary request.
   - If the block is empty or the request is unclear, review the conversation context for the user's intent.
   - If the request is vague or ambiguous, ask 1–2 clarifying questions before proceeding.
